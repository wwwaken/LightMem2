 ...
 *
 * Registry persistence is injected (`registryStore`). The default keeps the
 * registry in process memory, which is correct within a run; DSH-state-backed
 * persistence that survives restart is the remaining integration point (paired
 * with 观祥's G4 restart acceptance) and plugs in here without touching the cycle.
 */
...
type SkipReason =
  ...
  | "estimator-not-configured" | "applied" | "no-candidates" | "error";
...
/** Injectable registry persistence. Default = in-process memory (per run). */
export interface RegistryStore {
  load(sessionId: string): SessionTaskRegistry | Promise<SessionTaskRegistry>;
  persist(sessionId: string, registry: SessionTaskRegistry): void | Promise<void>;
}

function inProcessRegistryStore(): RegistryStore {
  const bySession = new Map<string, SessionTaskRegistry>();
  return {
    load: (sessionId) => bySession.get(sessionId) ?? createEmptySessionTaskRegistry(sessionId),
    persist: (sessionId, registry) => { bySession.set(sessionId, registry); },
  };
}

/**
 * Register the TokenPilot eviction handler on `agent/pre-step`. `registryStore`
 * defaults to an in-process store; a DSH-state-backed store is injected here for
 * cross-restart persistence.
 */
export function registerEvictionPreStep(
  ctx: DshPluginContext,
  config: TokenPilotDshConfig,
  registryStore: RegistryStore = inProcessRegistryStore(),
): void {
  ...
  ctx.on("agent/pre-step", async (payload: DshPreStepPayload, next: DshPreStepNext): Promise<DshPreStepDecision> => {
    ...
    try {
      ...
      await registryStore.persist(session.id, cycle.registry);
